...
## **Cleaning**
It's immediately obvious that there's a lot of bad data in here. The deck csv was a bit of a challenge. It started with 346,232 decks, with each card in the deck in it's own column. That means a whopping 30 coolumns just for the decklist. My first thought was to take all 30 card columns and combine their values into a tuple. That would allow for significantly less width on the dataframe and also easily allow for dropping duplicates later. After that, I put each tuple'd decklist into a new column I titled "card_list". Now all that was left was to drop the original 30 columns that contained 1 card each and move on. I was after this I realized that the data also included decks from many of Hearthstone's special gamemodes, so I just wanted the decks that were made for ranked play. Now I've got a 202,375 row data frame of just decks in the ranked play mode, it's done right? Wrong. Keeping duplicate deck lists didn't seem very important as there were many copies of popular deck lists that were uploaded with little to no ratings. I sorted the data by ratings and dropped all duplicate decks with lower ratings, bringing the final data size to 184,903 decks.
